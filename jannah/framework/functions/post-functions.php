			...
			foreach ( $post_authors as $author ) {
				...
				if( ! empty( $args['twitter'] ) && ! empty( $author->ID ) ) {
					...
					if( ! empty( $author_twitter ) ){
						$post_meta .= '
							<a href="'. esc_url( $author_twitter ) .'" class="author-twitter-link" target="_blank" rel="nofollow noopener" title="'. esc_html__( 'Follow on X', TIELABS_TEXTDOMAIN ) .'">
...
if( ! function_exists( 'tie_get_share_buttons' ) ) {
	...
	function tie_get_share_buttons( $share_position = false ) {
		...
		$share_buttons = array(
			...
			'twitter' => array(
				'url'   => 'https://x.com/intent/post?text={post_title}&amp;url={post_link}'.$share_twitter_username,
